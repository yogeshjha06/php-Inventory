...
<table>
        ...
        <tr>
            <td>Vendor Name</td>
            <td><?php echo" <input style='width:100%' type='text' name='name' value='$name11' readonly>";?></td>
        </tr>

        <tr>
            <td>Vendor Contact</td>
            <td><?php echo" <input style='width:100%' type='text' name='ph' value='$ph1' readonly>";?></td>
        </tr>
        <tr>
            <td>Vendor Address</td>
            <td><?php echo" <input style='width:100%' type='text' name='adr' value='$add' readonly>";?></td>
        </tr>
<form method="POST">
        ...
        <tr>
                <td>Price</td>
                <td><input style='width:100%' type='number' name='pc' placeholder='Enter Price'></td>
        </tr>
        <tr>
...
else if($n==3)//add button 
{
        ...
        if(isset($_POST['ok']))
        {
          //////////////////////////////////////////////////////////////
            ...
            $price=$_POST['pc'];//price input

            $sql = "SELECT `qun` FROM `item` WHERE `item`='$item'";
            $result = mysqli_query($con,$sql);
            $row = mysqli_fetch_array($result);
            $q1 = $row[0];//quantity
          ...
          if($q1>0 && $q1>=$qun)
          {
                    //submit insert code
                    $con=mysqli_connect("localhost","root","","bd");//connection
                    $sql1 = "SELECT * FROM `s1` WHERE `rno`='$rno'";
